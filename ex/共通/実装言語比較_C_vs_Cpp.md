# FSI実装言語比較：C vs C++（実装者目線）

本書は、FSI（Adapter → DataStore → Capability → Publisher / Accessor）を
...
**推奨**：本設計に対しては **「C ABI公開ヘッダ＋C++17内部実装（例外/RTTI無効・動的確保は初期化時のみ）」**、
つまり**既存printer-fwと同じハイブリッド**が最有力。理由は§6。純Cにする合理性があるのは
「C++17ツールチェーンが用意できないターゲットが確定している」場合のみ。
...
- **公開ヘッダ＝C ABI、内部実装＝C++17（例外/RTTI無効）**
- 根拠：
