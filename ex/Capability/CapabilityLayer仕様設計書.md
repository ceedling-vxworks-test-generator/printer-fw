...
- Publish優先度の判定
- Publisher LayerへのCapability通知

### 2.3 Capability Layerの非責務
- Registryの直接参照・更新（DataStore Layer）
- Snapshotの実コピー処理（MachineSnapshotReader）
- 状態の永続保持（DataStore Layerが正本）
- 購読者への実配信・配信的差分の管理（Publisher Layer）
- 現在値の参照提供（Accessor Layer）

## 3. 入出力仕様
### 3.1 入力
Capability Layerへの主な入力はDataStore LayerからのRegistry更新通知である。

入力内容：  
- 更新されたRegistry領域がどこかわかるもの

### 3.2 参照データ
Capability Layerは直接DataStore LayerのRegistryを読まず、SnapshotReaderを介して参照する。

### 3.3 出力
Capability LayerはPublisherへ生成済みCapabilityを渡す。
...
## 4. Capability生成方針
### 4.1 基本方針
Capability Layerは、DataStoreLayerから更新通知を受けたあと、必要なSnapshotを取得し、現在状態を生成する。
Registry更新通知  
    ↓  
必要Snapshot決定  
    ↓
...
> Capability Layer内でCap同士を相互参照させない。
...
- **閾値跨ぎ（Threshold）の判定は本層で行う**。連続値が閾値を跨いだ場合、
  ...
  （閾値の「跨ぎ判定」は判断であり本層の責務。配信タイミング制御はPublisher Layer）
- ErrorCap / JobCap に変化があった場合は、通常の変更通知に加えて明示イベントとしても通知する

## 5. Snapshot取得設計
Capability Layerは毎回すべてのRegistryを参照せず、更新内容に応じて必要なSnapshotのみ取得する。  
Snapshot取得対象は、RegistrtyDomainで指定する。(DataStore Layer参照)  
Snapshot取得時の排他はSnapshotReader側で行う。
ステータス管理モジュールはRegistryを直接ロックしない。

## 6. Capability生成設計
Capability Layerは取得したSnapshotを元にCapabilityを生成する。  
Capabilityの種類ごとに判定クラスが存在しており、それを用いてSnapshotからCapabilityの生成を行う。  
各判定クラスについては現在TBD

## 7. 差分検知設計
Capability Layerは生成したCapabilityと前回生成済のCapabilityを比較する。
差分がある場合、Capability変更検知通知をPublisher Layerに通知する。
...
| IF | 所属Layer | 概要 |
|--------|--------|--------|
| IMachineSnapshotReader | DataStore Layer | RegistryからSnapshotを取得するためのIF |
| IPublisher | Publisher Layer | 生成したCapabilityをPublisher層へ渡すためのIF |
...
| IF | 内容 |
|--------|--------|
| onRegistryUpdated(RegistryDomainSet& domains) | DataStore LayerからのRegistry更新通知を受け、ステータス生成処理を開始する。 |
...
本クラスはDataStore Layerが提供するSnapshot取得IFである。
CapabilityaManagerがRegistry情報を参照するために利用する。
| IF | 内容 |
|--------|--------|
| capture(SnapshotRequest request) | 指定されたRegistryDomainSetに対応するSnapshotを取得する。 |

詳細については、DataStore Layerの設計書を参照すること。

## 8.8 IPublisher
本クラスはPublisher Layerが提供するSnapshot取得IFである。
CapabilityManagerがPublisher LayerにPublish Triggerを通知するために用いる。
| IF | 内容 |
|--------|--------|
...
## 11. 排他・リアルタイム性設計
Capability LayerはRegistyを直接ロックしない。  
