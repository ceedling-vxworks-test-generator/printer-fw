...
/*
 * rim_capi_test.h - rim_api.h (C API) テスト用の最小ハーネス。
 *
 * このテストは「C言語から呼べること」自体が検証対象なので、C++専用の gtest は
 * 使えない。assert() は最初の失敗で即 abort してしまい1回の実行で1件しか分から
 * ないため、失敗を数えて続行する EXPECT 方式にしている。
 *
 * 使い方:
...
/* 浮動小数点の許容差付き比較 */
#define RIM_EXPECT_NEAR(actual, expected, tol)                                            \
    do {                                                                                  \
        double a_ = (double)(actual), e_ = (double)(expected), d_ = a_ - e_;               \
        if (d_ < 0) d_ = -d_;                                                             \
        if (d_ > (tol)) RIM_FAIL_("%s: actual=%.10f expected=%.10f", #actual, a_, e_);     \
    } while (0)
