...
RIM_TEST(FaultSnapshotRegistersActiveAndHealedEntries)
{
    ...
    RIM_EXPECT_EQ_I(RIManager_PushFaultSnapshot(handle, entries, 2), RI_SUCCESS);

    rim_error_capability_t error;
    RIM_EXPECT(RimWaitErrorCount(handle, 2, &error));
...
RIM_TEST(FaultSnapshotRegistersUnknownHealedCodeAsRecovered)
{
    ...
    RIM_EXPECT_EQ_I(RIManager_PushFaultSnapshot(handle, &entry, 1), RI_SUCCESS);

    rim_error_capability_t error;
    RIM_EXPECT(RimWaitErrorCount(handle, 1, &error));
...
RIM_TEST(FaultSnapshotRemovesCodeMissingFromNextSnapshot)
{
    ...
    RIM_EXPECT_EQ_I(RIManager_PushFaultSnapshot(handle, first, 2), RI_SUCCESS);
    ...
    RIM_EXPECT_EQ_I(RIManager_PushFaultSnapshot(handle, second, 1), RI_SUCCESS);
...
RIM_TEST(FaultSnapshotUpdatesStateOfCodeRegisteredViaAddError)
{
    ...
    RIM_EXPECT_EQ_I(RIManager_AddError(handle, 0x400), RI_SUCCESS);
...
RIM_TEST(FaultSnapshotEmptyListClearsAll)
{
    ...
    RIM_EXPECT_EQ_I(RIManager_AddError(handle, 0x500), RI_SUCCESS);

    rim_error_capability_t error;
    RIM_EXPECT(RimWaitErrorCount(handle, 1, &error));

    RIM_EXPECT_EQ_I(RIManager_PushFaultSnapshot(handle, NULL, 0), RI_SUCCESS);

    RIM_EXPECT(RimWaitErrorCount(handle, 0, &error));
...
RIM_TEST(FaultSnapshotInvalidArgumentsAreRejected)
{
    ...
    RIM_EXPECT_EQ_I(
        RIManager_PushFaultSnapshot(NULL, &entry, 1),
        RI_INVALID_HANDLE);

    /* NULL 配列で count != 0 は拒否される。 */
    RIM_EXPECT_EQ_I(
        RIManager_PushFaultSnapshot(handle, NULL, 1),
    ...
    /* 内部上限(kMaxErrors=32)を超える count は、黙って切り詰めず拒否する。 */
    {
        ...
        RIM_EXPECT_EQ_I(
            RIManager_PushFaultSnapshot(handle, many, 33),
            RI_INVALID_PARAMETER);
    }
