/**
 * @file test_printer_fw.cpp
 * @brief printer-fw コアの最小ユニットテスト（実アサーション付き）。
 *
 * 依存を増やさないため自己完結のCHECKマクロで書いているが、構造は Catch2/GoogleTest や
 * Unity/ceedling にそのまま移植できる（TEST_ASSERT_* 等に置換）。
 *
...
void test_data()
{
    ...
    /* 予約ID 0xFFFF は弾く */
    const pf_data_desc_t bad[] = { { 0xFFFF, PF_TYPE_U32, 0, 1, 0, "x" } };
...
{
    std::printf("[test_fsm]\n");
    const pf_fsm_transition_t tr[] = { {0,1}, {1,0}, {1,2} };
    const pf_fsm_desc_t f = { 0, 0, tr, 3, "f" };
...
int        g_notes = 0;
pf_event_t g_last{};
void cb(const pf_event_t* ev, void* ctx) { (void)ctx; g_notes++; g_last = *ev; }

void test_monitor_observer()
{
    std::printf("[test_monitor_observer]\n");
    pf_core_deinit();
    pf_port_t port = pf_port_baremetal();
    CHECK(pf_core_init(model_sample_get(), &port) == PF_OK);

    pf_subscription_t h;
    CHECK(pf_observer_subscribe(SAMPLE_STATE_TEMP_ALERT, cb, nullptr, &h) == PF_OK);
...
void test_volume_type_reconciliation()
{
    std::printf("[test_volume_type_reconciliation]\n");
    pf_core_deinit();
    pf_port_t port = pf_port_baremetal();
    CHECK(pf_core_init(model_sample_get(), &port) == PF_OK);
    ...
    CHECK(model_sample_volume_ingest_from_short(static_cast<short>(30)) == PF_OK);
    ...
    CHECK(model_sample_volume_ingest_from_short(static_cast<short>(10)) == PF_OK);
    CHECK(pf_monitor_tick() == PF_OK);
    int32_t level = -1;
    ...
    CHECK(model_sample_volume_ingest_from_short(static_cast<short>(90)) == PF_OK);
    CHECK(pf_monitor_tick() == PF_OK);
    CHECK(pf_state_get(SAMPLE_STATE_VOLUME_LEVEL, &level) == PF_OK);
...
int main()
{
    std::printf("=== printer-fw unit tests ===\n");
    test_data();
    test_fsm();
