...
#include "test/core/support/TestRIMIdConverter.hpp"
    ...
    GetData)
{
    ...
    EXPECT_TRUE(
        accessor.TryGetData(
            rim::test::DataId(RI_DATA_TEMPERATURE_SENSOR_A),
            out));
    ...
    ReturnFalseWhenDataNotFound)
{
    ...
    EXPECT_FALSE(
        accessor.TryGetData(
            rim::test::DataId(rim::test::kUnknownDataId),
            out));
}
    ...
    GetBinaryInfo)
{
    ...
    ASSERT_TRUE(
        accessor.GetBinaryInfo(
            rim::test::DataId(RI_DATA_ERROR_LIST),
            info));
